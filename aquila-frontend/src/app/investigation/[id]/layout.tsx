  ...
  return (
    <div className="flex flex-col h-full w-full bg-surface-container-lowest">
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col">
        {(isLoading || authLoading) && (!investigation || investigation.id !== id) && (
          <div className="absolute inset-0 z-50 bg-surface flex items-center justify-center flex-col gap-4">
            <div className="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
            <p className="font-mono text-sm text-on-surface-variant">Loading Investigation: {id}...</p>
          </div>
        )}
        {error && !authLoading && (!investigation || investigation.id !== id) && (
          <div className="absolute inset-0 z-50 bg-surface flex items-center justify-center flex-col gap-4 p-8 text-center">
