...
class MockAISProvider(AISProvider):
    ...
    async def fetch_raw_positions(self, min_lat: float, min_lon: float, max_lat: float, max_lon: float,
        ...
        for i in range(-12, 13):
            ...
            t = self.release_time + timedelta(hours=i)
            if start_time <= t <= end_time:
                p = AISPosition(timestamp=t, lon=self.target_lon + (i * 0.1),
                                lat=self.target_lat, speed_knots=12.0, heading=90.0)
        ...
        for i in range(-12, 13):
            t = self.release_time + timedelta(hours=i)
            if start_time <= t <= end_time:
                p = AISPosition(timestamp=t, lon=self.target_lon + 0.2, lat=self.target_lat +
                                ((i - 2) * 0.15), speed_knots=15.0, heading=0.0)
...
class AISService:
    ...
    async def discover_candidates(self, origin: OriginEstimate, start_time: datetime,
                                  end_time: datetime) -> List[VesselCandidate]:
        """
